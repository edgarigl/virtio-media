...
/*
 * Base response size without the optional grant-reference fields, i.e.
 * {hdr, driver_addr, len}. Devices that have not negotiated
 * VIRTIO_MEDIA_F_GNTREF reply with exactly this many bytes; the driver must
 * not require the full sizeof() in that case.
 */
